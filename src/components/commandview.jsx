...
export default function Commandeview({handleClose, commande, setOrders, setlinkreset, linkreset}) {
  ...
  const getOrders = async () => {
    var response = await axios.get(`http://localhost:3001/commandes/proprietaire/${commande.id_proprietaire}`);
 ...
 React.useEffect(() => {
  ...
  function Accept(){
    ...
    document.getElementById('confirmation').style.display='inline'
      ...
      axios.put(`http://localhost:3001/update/commande/${commande.id_commande}`, order).then(res => {
        console.log(res);
