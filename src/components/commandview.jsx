...
export default function Commandeview({handleClose, commande, setOrders, setlinkreset, linkreset}) {
  ...
  const getOrders = async () => {
    var response = await axios.get(`https://photouploadobisto.onrender.com/commandes/proprietaire/${commande.id_proprietaire}`);
 ...
 React.useEffect(() => {
  ...
  function Accept(){
    ...
    document.getElementById('confirmation').style.display='inline'
      ...
      axios.put(`https://photouploadobisto.onrender.com/update/commande/${commande.id_commande}`, order).then(res => {
        console.log(res);
